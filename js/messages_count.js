	...
	function onResponseGet(r) {
		...
		for (let i = 0; i < r.response.items.length; i++) {
			...
			if (r.response.items[i].from_id == user_id) {
				let from_user = r.response.items[i].from_id == user_id;
				let in_conversation = r.response.items[i].peer_id >= 2000000000;
				if (Args.consider.from_user_conversations && in_conversation && from_user) {
					dataset[part_index]++;
				} else if (Args.consider.from_others_conversations && in_conversation && !from_user) {
					dataset[part_index]++;
				} else if (Args.consider.from_user && (!in_conversation) && from_user) {
					dataset[part_index]++;
				} else if (Args.consider.from_others && (!in_conversation) && !from_user) {
					dataset[part_index]++;
				}
			}
		}
