// Источник: http://vkscripts.ru/js/vk.js
...
var module_vk = function(access_token) {
    ...
    var vk = {
        ...
        upload: function(first_method, save_method, data, cb) {
            ...
            var _data = vk.other.collect(data, {
                files: ""
            });
        ...
        sreq: {
            ...
            jsonp: function(url, d, onSuccess, onError) { //Стандартный метод запросов на другой домен для браузера
                if (!onError) onError = function(a) {
                    console.log(a)
                };
                if (!onSuccess) onSuccess = function(a) {
                    console.log(a)
                };
                var scriptOk = false;
            ...
            ext: {
                get: function(u, d, c, e) {
                    if (typeof e !== "function") e = function(b) {
                        console.log(b)
                    };
                    if (typeof c !== "function") c = function(b) {
                        console.log(b)
                    };
        ...
        _api: function(method, data, callback, place) {
            ...
            vk.default_method(url, data, function(body) {
                ...
                if (data["error"]) callback({
                    error: {
                        error_code: 404,
                        error_msg: "Нет соединения с интернетом"
                    }
                });
                else vk.on.error({
                    error_code: 404,
            ...
            messages: function(_cb, data) {
                ...
                vk.api.messages.getLongPollServer({
                    ...
                    setInterval(function() {
                        vk.api.messages.getLongPollHistory({
                            pts: pts,
                            access_token: data.access_token,
                            fields: data.fields,
                            lang: data.lang
                            ...
                            delete res.response.profiles.map(function(profile) {
                                users_info[profile.id] = profile;
                            });
        ...
        on: {
            captcha: function(_error, _m, _q, _cb) {
                if (vk.debug) console.info("Был запрошен защитный код, игнорируем...", _error);
            },
